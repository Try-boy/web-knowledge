...
当prop 验证失败的时候，(开发环境构建版本的) Vue 将会产生一个控制台的警告
...
如上述，我们要自定义一个input组件，我们想对input做一些扩展，但是又不想改变原生的使用方式
...
注意：我们亲爱的文档说，ref 本身是作为渲染结果被创建的，在初始渲染的时候你不能访问它们 - 它们还不存在，所以上文是在mounted生命周期中获取的
